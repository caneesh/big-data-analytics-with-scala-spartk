...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  /** To compute the vectors for K-means */    
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.flatMap {
      case (question, highScore) => question.tags.flatMap {
        case tag => firstLangInTag(Some(tag), langs).map {
          case idx => (idx * langSpread, highScore)
      }
    }
  }
}
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = closest
    .groupByKey()
    .mapValues(points => averageVectors(points))
    .collectAsMap()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      val langCounts = vs.map { case (langIndex, _) => (langs(langIndex / langSpread), 1) }
      .groupBy(_._1)
      .mapValues(_.map(_._2).sum)  
      val mostCommonLang = langCounts.maxBy(_._2)
      val langLabel = mostCommonLang._1
      val langPercent = mostCommonLang._2.toDouble / vs.size * 100
      val clusterSize = vs.size
      val medianScore: Int    = calculateMedian(vs.map(_._2))

      (langLabel, langPercent, clusterSize, medianScore)
    }

    median.collect().map(_._2).sortBy(_._4)
  // Helper function to calculate the median of a sequence of integers
def calculateMedian(scores: Iterable[Int]): Int = {
  val sortedScores = scores.toList.sorted
  val size = sortedScores.size
  if (size % 2 == 0) {
    val mid = size / 2
